# Source: https://docs.opencv.org/4.x/d9/dbc/tutorial_generic_corner_detector.html
# 30/10/2022
import cv2 as cv
import argparse
import numpy as np
    ...
    for i in range(src_gray.shape[0]):
        for j in range(src_gray.shape[1]):
            if myShiTomasi_dst[i, j] > myShiTomasi_minVal + (myShiTomasi_maxVal - myShiTomasi_minVal)*myShiTomasi_qualityLevel/max_qualityLevel:
                cv.circle(myShiTomasi_copy, (j, i), 4, (rng.randint(
                    0, 256), rng.randint(0, 256), rng.randint(0, 256)), cv.FILLED)
...
cv.createTrackbar('Quality Level: ', myShiTomasi_window,
