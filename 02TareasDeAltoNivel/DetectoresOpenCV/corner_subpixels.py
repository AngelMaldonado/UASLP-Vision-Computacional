# Source: https://docs.opencv.org/4.x/dd/d92/tutorial_corner_subpixels.html
# 30/10/2022
import argparse
import cv2 as cv
import numpy as np
...
def goodFeaturesToTrack_Demo(val):
    ...
    print('** Numbers of corners detected: ', corners.shape[0])
    radius = 4
    for i in range(corners.shape[0]):
        cv.circle(copy, (int(corners[i, 0, 0]), int(corners[i, 0, 1])), radius, (rng.randint(
...
parser = argparse.ArgumentParser(
    description='Code for Shi-Tomasi corner detector tutorial.')
...
cv.createTrackbar('Threshold: ', source_window, maxCorners,
